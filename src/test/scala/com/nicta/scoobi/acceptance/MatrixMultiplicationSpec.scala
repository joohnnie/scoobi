...
class MatrixMultiplicationSpec extends NictaSimpleJobs with ScalaCheck {
  ...
  def run()(implicit sc: ScoobiConfiguration) = Prop.forAll(genMatrixData, genMatrixData)(runTest)
  ...
  def runTest(matrix1: Iterable[MatrixEntry], matrix2: Iterable[MatrixEntry])(implicit sc: ScoobiConfiguration): Boolean = {
    ...
    val res2 = toDMatrix(matrix1) byMatrixWithoutCombiner (toDMatrix(matrix2), mult, add) // hadoop bug workaround for large matrices
    val res3 = toApacheMatrix(matrix1).multiply(toApacheMatrix(matrix2)) // sanity check

    toEntrySet(res) == toEntrySet(res2) && toEntrySet(res2) == toEntrySet(res3)
  }
